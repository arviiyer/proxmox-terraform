...
- **Submit URL status (2026-03-31):** Linux `Submit URL` is working. Windows `Submit URL` backend orchestration is now fully fixed — job completes cleanly, artifacts staged, interactive session confirmed before staging. Remaining blocker: `schtasks /Run /IT /RU analyst` returns SUCCESS but Edge does not visibly open in the VNC console. Probe testing (manual qm guest exec) confirmed the mechanism works in isolation (14 Edge processes in Session 1). Next debugging step: run `Get-Process msedge` immediately after staging via a separate guest exec to determine whether Edge is actually running but invisible, or whether the schtask is silently failing in the clone context.
...
- **PVE 9.x guest exec returns integer exited field:** `qm guest exec` on PVE 9.x is synchronous — it blocks until the command exits and returns `{"exited": 1, "exitcode": N, ...}` inline (integer `1`, not JSON `true`). The `guestExecResult.Exited` field must be `int`, not `bool`. If typed as `bool`, `json.Unmarshal` fails, the immediate-result check is skipped, PID parsing finds no `pid` field, and every exec returns "guest exec did not return a pid". Do not change `Exited` back to `bool`.
- **DHCP fires before Windows auto-logon:** `waitForGuestNetworkReady` returns when the DHCP client assigns an IP — this happens before the user's interactive session (auto-logon / Explorer.exe) is fully established. For Windows `Submit URL`, `schtasks /Run /IT /RU analyst` silently does nothing if called before Explorer is up. Always call `waitForGuestInteractiveSession` (polls `Get-Process explorer`) before invoking Windows URL staging.
- **summerset memory headroom:** permanent VMs on summerset total ~21 GB (c9-server 4 GB, srv-apps 12 GB, srv-pbs 4 GB, srv-security 1 GB). A Windows sandbox VM adds 8 GB = 29 GB of 31 GB physical. Balloon driver is enabled in `sandbox-infra/main.tf` (`floating = 0`) so Proxmox can reclaim unused guest RAM. If swap fills up again run `swapoff -a && swapon -a` on summerset (safe when free RAM > swap used). Long-term fix: migrate srv-apps or srv-pbs to valenwood.
- **Windows URL handoff reality:** Edge first-run/sync onboarding can intercept a freshly launched Windows sandbox and hide the submitted URL behind setup screens. The staging now invokes `msedge.exe --no-first-run --no-default-browser-check` directly via schtask to bypass this.

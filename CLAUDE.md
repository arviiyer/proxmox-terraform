...
- **Internet-mode live status (2026-03-30):** `vmbr2` now exists on summerset via `nic0.60`, host firewall backstops now block `vmbr2` from `vmbr0`/`vmbr1`, OPNsense now has `SANDBOX_PUBLIC` (`opt5`, `10.60.0.1/24`) with public-only deny-first rules, and the Unifi switch now tags VLAN 60 on the OPNsense uplink (port 16) and summerset port (port 2). A disposable guest on `vmbr2` successfully received `10.60.0.100/24`, reached `10.60.0.1`, and reached public internet (`1.1.1.1`), while pings to `10.0.0.11`, `10.0.0.83`, and `100.64.0.1` failed as intended. Residual caveat: `10.0.0.1` still answers ICMP from `SANDBOX_PUBLIC`, so tighten that separately if the firewall itself must be completely dark to the detonation segment.
